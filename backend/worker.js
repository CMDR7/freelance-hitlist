...
function notFound(path) {
  return json({ error: "NOT_FOUND", path, message: "FL-HL Intelligence API route not found." }, 404);
}
...
async function fetchFeed(feed) {
  ...
  try {
    const response = await fetch(feed.url, {
      headers: {
        "User-Agent": "FL-HL-Intelligence-Network/2.7 (+public-feed-ingestion)",
    ...
    if (!/<(?:rss|feed|rdf:RDF)\b/i.test(xml)) throw new Error("Response does not appear to be an RSS/Atom/XML feed");
...
function normalizeJobicy(job, retrievedAt) {
  return {
    sourceId: "jobicy",
    sourceName: "Jobicy Public Jobs API",
    title: job.jobTitle || "Untitled opportunity",
    employer: job.companyName || "Unknown employer",
    url: job.url,
    engagement: asArray(job.jobType),
    location: asArray(job.jobGeo || "Remote"),
    tags: [...asArray(job.jobIndustry), job.jobLevel].filter(Boolean),
    publishedAt: job.pubDate || null,
    retrievedAt,
    status: "active",
    description: stripHtml(job.jobExcerpt || job.jobDescription || ""),
    attribution: CONNECTORS.jobicy.attribution,
    acquisition: "api",
    stage: "ingested-candidate",
  };
}
...
function normalizeArbeitnow(job, retrievedAt) {
  ...
  return {
    sourceId: "arbeitnow",
    sourceName: "Arbeitnow Free Job Board API",
    title: job.title || "Untitled opportunity",
    employer: job.company_name || job.company || "Unknown employer",
    url: job.url,
    engagement: asArray(job.job_types || job.job_type || job.type),
    location: [remote ? "Remote" : null, job.location].filter(Boolean),
    tags: asArray(job.tags || job.category),
    publishedAt: job.created_at || job.published_at || null,
    retrievedAt,
    status: "active",
    description: stripHtml(job.description || ""),
    attribution: CONNECTORS.arbeitnow.attribution,
    acquisition: "api",
    stage: "ingested-candidate",
  };
}
...
async function fetchConnector(name) {
  ...
  const response = await fetch(connector.url, {
    headers: { Accept: "application/json", "User-Agent": "FL-HL-Intelligence-Network/2.7" },
  });
...
async function acquireAll() {
  ...
  const api = apiResults.map((result, index) => result.status === "fulfilled"
    ? result.value
    : { connector: CONNECTORS[Object.keys(CONNECTORS)[index]].id, source: Object.keys(CONNECTORS)[index], retrievedAt: new Date().toISOString(), count: 0, stage: "ingested-candidate", data: [], error: String(result.reason?.message || result.reason) });

  return {
    candidates: [...rss.items, ...api.flatMap((result) => result.data)],
    rss,
    api,
  };
}
...
async function handleRequest(request, env) {
  ...
  if (path === "/" || path === "/api") {
    return json({
      ...
      version: API_VERSION,
      status: "operational",
      mode: "normalized-ingestion",
      endpoints: ["/api/health", "/api/feeds", "/api/connectors", "/api/ingest/rss", "/api/ingest/api?connector=jobicy", "/api/ingest/api?connector=arbeitnow", "/api/opportunities?live=true", "/api/sources", "/api/sync/status"],
      dataSource: "public-rss-and-free-apis",
  ...
  if (path === "/api/health") {
    return json({
      ...
      apiConnectors: Object.keys(CONNECTORS).length,
    });
  }
  ...
  if (path === "/api/ingest/rss") {
    ...
    if (feedId && !FEEDS.some((feed) => feed.id === feedId)) return json({ error: "UNKNOWN_FEED", feed: feedId }, 404);
  ...
  if (path === "/api/ingest/api") {
    const connector = (url.searchParams.get("connector") || "").trim().toLowerCase();
    if (!connector) return json({ error: "CONNECTOR_REQUIRED", available: Object.keys(CONNECTORS) }, 400);
    try {
      return json(await fetchConnector(connector));
    } catch (error) {
      return json({ error: "CONNECTOR_FAILED", connector, message: error instanceof Error ? error.message : String(error) }, 502);
    }
  }
  ...
  if (path === "/api/opportunities") {
    ...
    if (!live) {
      return json({
        ...
        query: q,
        region,
        statusCode: "READY_FOR_LIVE_DISPLAY",
        message: "Use live=true to acquire, normalize, and deduplicate current candidates in V2.7.",
      });
    }
  ...
  if (path === "/api/sync/status") {
    return json({
      status: "manual-ingestion",
      lastRunAt: null,
      lastSuccessfulRunAt: null,
      sourcesChecked: FEEDS.length + Object.keys(CONNECTORS).length,
